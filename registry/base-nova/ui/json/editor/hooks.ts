...
import { toNode, type EditorState } from "@/registry/base-nova/ui/json/core/editor";
import { validate, type Issue } from "@/registry/base-nova/ui/json/core/validate";
import type { Field, FieldPatch, Json, TypeModule } from "@/registry/base-nova/ui/json/core";
...
const toField = (s: Tree, id: string): Field => {
  const n = s.byId[id];
  return n.isGroup ? { ...n, fields: (s.children[id] ?? []).map((c) => toField(s, c)) } : { ...n };
};
...
export function useSchema() {
  ...
  const schema = React.useMemo(() => toField({ byId, children }, root), [byId, children, root]);
  const selectedFields = React.useMemo(
    () => selected.filter((id) => byId[id]).map((id) => toField({ byId, children }, id)),
    [selected, byId, children],
  );
  const issues = React.useMemo(() => issuesOf({ byId, children, root }), [byId, children, root]);
  ...
  const fields = React.useMemo(
    () => ({
      ...
      drop: (refs: string | Field | (string | Field)[]) => s().remove(idsOf(refs)),
