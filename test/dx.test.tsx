...
describe("preset", () => {
  test("one line: handle in, rows out, JSON back", async () => {
    ...
    render(<Owner />);
    expect(
      screen.getAllByRole("textbox", { name: "Title" }).map((e) => (e as HTMLInputElement).value),
  ...
  test("mobile is automatic on a coarse pointer, overridable", () => {
    ...
    try {
      ...
      expect(
        screen.getAllByText("ID")[0].closest("[data-slot=row]")!.getAttribute("data-variant"),
      ).toBe("mobile");
      unmount();
...
describe("composing parts", () => {
  ...
  test("parts take a look variant: Type badge, input fields, badge looks", () => {
    ...
    expect(within(row(titleOf("Address"))).getByText("2 fields").dataset.variant).toBe("secondary");
  });

  test("EditDetails from a closing menu still opens the dialog", async () => {
    render(<JsonSchemaEditor schema={createJsonSchema(user)} variant="compact" />);
    const name = row(titleOf("Name"));
    await userEvent.click(within(name).getByRole("button", { name: /field settings/i }));
  ...
  test("SchemaAction.Command: your own action, targets from scope", async () => {
    ...
    render(
      <Schema.Root store={schema}>
        <Schema.Toolbar>
          <SchemaAction.Command onClick={({ nodes }) => seen.push(nodes.map((n) => n.key))}>
            Export
        ...
        <Schema.List
          render={() => (
            <SchemaField.Row>
              ...
              <SchemaField.Title />
              <SchemaAction.Command
                onClick={({ ids, schema }) => schema.update(ids[0], { title: "Zapped" })}
              >
                Zap
              </SchemaAction.Command>
            </SchemaField.Row>
  ...
  test("hooks: useSchema() is the tree + ops by slug; useField() is the row", async () => {
    ...
    function MyRow() {
      ...
      return (
        <>
          ...
          <button onClick={() => f.update({ optional: !f.field.optional })}>opt</button>
    ...
    expect(api!.fields.get("address")!.fields!.map((f) => f.key)).toEqual(["zip"]);
    expect(api!.fields.get("address.zip")!.title).toBe("Postcode");
    expect(api!.selectedFields.map((f) => f.key)).toEqual(["address", "name"]);
    act(() => api!.setSchema({ type: "object", properties: { a: { type: "string" } } }));
    expect(out()).toBe("a");
  });
  ...
  test("Schema.Column: per-row cells beside the template, inherited by nested lists", async () => {
    ...
    const kids = Array.from(street.children).map((c) => (c as HTMLElement).dataset.slot);
    expect(kids).toEqual(["columns", "row-content", "columns"]);
    expect(street.querySelector("[data-slot=card] [data-slot=select]")).toBeNull();
    await userEvent.click(within(street).getByRole("checkbox"));
    expect(schema.selected()).toEqual([schema.find("address.street")!]);
    await userEvent.click(within(street).getByRole("button", { name: /remove/i }));
        ...
        <Schema.List
          render={(node) => (
            <SchemaField.Row>
              ...
              {node.isGroup && (
                <SchemaField.ChildrenCount
                  render={(props, { count }) => <b {...props}>{String(count)} champs</b>}
                />
              )}
            </SchemaField.Row>
          )}
...
describe("selection + toolbar", () => {
  test("shift-click on Select extends the selection in document order", async () => {
    ...
    expect(schema.selected()).toEqual(
      ["name", "address", "address.street", "address.zip"].map((p) => schema.find(p)!),
    );
  ...
  test("MoveInto lists groups, moves the selection into the pick", async () => {
    ...
    await userEvent.click(within(menu).getByRole("menuitem", { name: "Address" }));
    const json = schema.toJSON() as any;
    ...
    expect(schema.selected()).toEqual([schema.find("address.id")!, schema.find("address.name")!]);
  });
